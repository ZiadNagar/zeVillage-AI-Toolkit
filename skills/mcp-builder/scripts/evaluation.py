...
This script evaluates MCP servers by running test questions against them using any
LLM provider supported by litellm (OpenAI, Google Gemini, Azure, Ollama, and more).
"""
...
def convert_tools_to_openai_format(tools: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Convert MCP tools to OpenAI/litellm tool format."""
    return [
        {
            "type": "function",
            "function": {
...
async def evaluate_single_task(
    ...
    response, tool_metrics = await agent_loop(
        model, qa_pair["question"], tools, connection
    )
...
async def run_evaluation(
    ...
    for i, qa_pair in enumerate(qa_pairs):
        ...
        result = await evaluate_single_task(model, qa_pair, tools, connection, i)
...
Examples:
  ...
  python evaluation.py -t http -u https://example.com/mcp -m gpt-4o eval.xml
    ...
    parser.add_argument(
        ...
        "--model",
        default="gpt-4o",
        help="LLM model to use in litellm format, e.g. gpt-4o, gemini/gemini-2.0-flash, ollama/llama3 (default: gpt-4o)",
    )
