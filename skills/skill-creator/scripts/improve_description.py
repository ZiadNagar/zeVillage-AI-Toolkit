...
using an LLM with extended thinking (when supported by the model).
"""
...
The description appears in the agent's "available_skills" list. When a user sends a query, the AI agent decides whether to invoke the skill based solely on the title and on this description. Your goal is to write a description that triggers for relevant queries, and doesn't trigger for irrelevant ones.
...
- The description competes with other skills for the AI agent's attention — make it distinctive and immediately recognizable.
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    # Enable extended thinking when explicitly requested (model must support it)
    if extended_thinking:
        kwargs["thinking"] = {"type": "enabled", "budget_tokens": 10000}

    response = litellm.completion(**kwargs)
    msg = response.choices[0].message
    thinking_text = getattr(msg, "thinking", "")
    text = msg.content or ""
    ...
    if len(description) > 1024:
        ...
        if extended_thinking:
            shorten_kwargs["thinking"] = {"type": "enabled", "budget_tokens": 10000}

        shorten_response = litellm.completion(**shorten_kwargs)
        shorten_msg = shorten_response.choices[0].message
        shorten_thinking = getattr(shorten_msg, "thinking", "")
        shorten_text = shorten_msg.content or ""
...
def main():
    ...
    parser.add_argument(
        "--extended-thinking",
        action="store_true",
        help="Enable extended thinking (model must support it)",
    )
    parser.add_argument(
